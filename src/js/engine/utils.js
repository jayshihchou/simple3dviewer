import { vec4 } from '../lib/gl-matrix/index.js';
...
const inverseDevicePixelRatio = isMobile ? 1.0 / devicePixelRatio : 1.0;
...
function mulPoint(mat, p) {
  const v0 = vec4.set(vec4.create(), mat[0], mat[1], mat[2], mat[3]);
  const v1 = vec4.set(vec4.create(), mat[4], mat[5], mat[6], mat[7]);
  const v2 = vec4.set(vec4.create(), mat[8], mat[9], mat[10], mat[11]);
  const v3 = vec4.set(vec4.create(), mat[12], mat[13], mat[14], mat[15]);
  vec4.scale(v0, v0, p[0]);
  vec4.scale(v1, v1, p[1]);
  vec4.scale(v2, v2, p[2]);
  vec4.add(v0, v0, v1);
  vec4.add(v0, v0, v2);
